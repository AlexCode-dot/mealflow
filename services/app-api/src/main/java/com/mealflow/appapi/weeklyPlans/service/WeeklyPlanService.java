...
public class WeeklyPlanService {
  ...
  private static final Set<String> WEEK_DAYS = new HashSet<>(
  ...
  public WeeklyPlan create(String userId, String weeklyStart, List<PlanEntry> entries) {
    String normalizedWeeklyStart = normalizeWeeklyStart(weeklyStart);
    List<PlanEntry> normalizedEntries = normalizeEntries(entries);
    validateEntries(normalizedEntries, userId);

    Instant now = clock.instant();
    WeeklyPlan plan = new WeeklyPlan(userId, normalizedWeeklyStart, normalizedEntries, now, now);
    return weeklyPlanRepository.save(plan);
  }

  public WeeklyPlan patch(
      String userId, String planId, String weeklyStart, List<PlanEntry> entries) {
    WeeklyPlan existing = getForUser(userId, planId);

    String normalizedWeeklyStart =
        weeklyStart == null ? null : normalizeWeeklyStart(weeklyStart);
    ...
    existing.applyPatch(normalizedWeeklyStart, normalizedEntries, clock.instant());
    return weeklyPlanRepository.save(existing);
  }

  public WeeklyPlan replace(String userId, String planId, String weeklyStart, List<PlanEntry> entries) {
    WeeklyPlan existing = getForUser(userId, planId);
    String normalizedWeeklyStart = normalizeWeeklyStart(weeklyStart);
    List<PlanEntry> normalizedEntries = normalizeEntries(entries);
    validateEntries(normalizedEntries, userId);
  ...
  private void validateEntries(List<PlanEntry> entries, String userId) {
    for (PlanEntry entry : entries) {
      ...
      boolean hasRecipe = entry.getRecipeId() != null && !entry.getRecipeId().isBlank();
      boolean hasCustomTitle = entry.getCustomTitle() != null && !entry.getCustomTitle().isBlank();
      if (!hasRecipe && !hasCustomTitle) {
        throw new WeeklyPlanValidationException(
            "Each entry must reference a recipe or provide a custom title");
      }
      if (hasRecipe && hasCustomTitle) {
        throw new WeeklyPlanValidationException(
            "Entries cannot have both recipeId and customTitle");
      }
    }
    ...
    int found = recipeRepository.findAllByIdInAndUserId(List.copyOf(recipeIds), userId).size();
